...
# - NEW: supports ROI loss around the first deep dip / post-dip spike region.
# - UPDATED: ROI loss is now applied on ALL channels, not only magnitude channel.
# This is important because the missed local resonance feature appears in both
# magnitude and phase.
...
# ==========================================
# 2. Helper: build frequency weights
# ==========================================
...
# ==========================================
# 4. Helpers: train-only stats + normalization
# ==========================================
@torch.no_grad()
...
# 5. Helpers: ROI loss
# ==========================================
def _build_roi_mask_from_target_ln(
    target_ln: torch.Tensor,
    roi_left: int,
    roi_right: int,
    roi_weight: float,
) -> torch.Tensor:
    """
    Build per-sample ROI mask around first deep dip / main dip region.

    Args:
    target_ln : [B, F] normalized or physical ln|Z|, either is fine for argmin index
    roi_left : points to include before dip
    roi_right : points to include after dip
    roi_weight: extra weight inside ROI window

    Returns:
    roi_mask : [B, F] with base=1 outside ROI and roi_weight inside ROI
    """
    if target_ln.ndim != 2:
        raise ValueError(f"Expected target_ln shape [B, F], got {tuple(target_ln.shape)}")

    B, F = target_ln.shape
    device = target_ln.device

    roi_mask = torch.ones((B, F), dtype=torch.float32, device=device)

    # Use main dip (argmin of ln|Z|) as anchor
    dip_idx = torch.argmin(target_ln, dim=1) # [B]

    for b in range(B):
        i = int(dip_idx[b].item())
        start = max(0, i - int(roi_left))
        end = min(F, i + int(roi_right))
        roi_mask[b, start:end] = float(roi_weight)

    return roi_mask


def _roi_all_channel_loss(
    pred: torch.Tensor,
    target: torch.Tensor,
    roi_left: int,
    roi_right: int,
    ...
    """
    ROI-weighted MSE on ALL channels.

    Args:
    pred, target : [B, C, F]
    roi_left, roi_right, roi_weight : ROI config
    ...
    # Use magnitude channel only to locate the dip
    targ_ln = target[:, 0, :] # [B, F]

    roi_mask = _build_roi_mask_from_target_ln(
    target_ln=targ_ln,
    roi_left=roi_left,
    roi_right=roi_right,
    ...
    # Expand ROI mask to all channels: [B, 1, F] -> broadcast over C
    roi_mask = roi_mask.unsqueeze(1) # [B, 1, F]

    loss = roi_mask * (pred - target) ** 2
    return loss.mean()
...
def train_model(
    ...
    # NEW: ROI-loss controls
    use_roi_loss = bool(config.get("use_roi_loss", False))
    lambda_roi = float(config.get("lambda_roi", 0.0))
    roi_left = int(config.get("roi_left", 3))
    roi_right = int(config.get("roi_right", 30))
    roi_weight = float(config.get("roi_weight", 3.0))
    ...
    # if weighted loss is used, print the actual weights for transparency
...
# ROI transparency
    if use_roi_loss and lambda_roi > 0.0:
        print(
            f"ROI loss active (ALL channels): lambda_roi={lambda_roi}, "
            f"roi_left={roi_left}, roi_right={roi_right}, roi_weight={roi_weight}"
        )
    ...
    for epoch in range(epochs):
        ...
        for x, _, _ in train_loader:
            ...
            # ROI loss around difficult local resonance region
            if use_roi_loss and lambda_roi > 0.0:
                loss_roi = _roi_all_channel_loss(
                    pred=recon_norm,
                    target=x_norm,
                    roi_left=roi_left,
                    roi_right=roi_right,
                ...
                loss = loss_recon + lambda_roi * loss_roi
